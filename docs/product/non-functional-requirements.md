...
| NFR-008 | Portability | Android and iOS are both targeted from day one via Expo. | No platform-only assumptions in shared layer |
| NFR-009 | Maintainability | Agent tools and schemas are typed and unit-testable. | Pydantic AI typed tools; unit tests present |
| NFR-010 | Cost | Backend fits a low-cost footprint suitable for coursework. | Runs on minimal Azure Container Apps sizing |
